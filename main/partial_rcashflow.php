...
foreach($_POST['total'] as $amount) {
    ...
    if (is_numeric($amount)) {
        ...
        echo "<br/>Sub Total amount : " . $totalAmount;
    }
}
...
if($ptype == "credit" ) {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $insertQuery = "INSERT INTO retcustomer_credit (customer_id,customer_name, credit) VALUES (:customer_id,:customer_name, :credit)";
...
if($ptype == "cash" ) {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        // The customer_id doesn't exist; insert a new record
        $insertQuery = "INSERT INTO retcustomer_credit (customer_id,customer_name, cash) VALUES (:customer_id,:customer_name, :cash)";
        $stmt = $db->prepare($insertQuery);
        $stmt->bindParam(':customer_id', $suplier_id, PDO::PARAM_INT);
         $stmt->bindParam(':customer_name', $cname, PDO::PARAM_STR);
        $stmt->bindParam(':cash', $totalAmount, PDO::PARAM_INT);
...
try {
    ...
    $sql = "INSERT INTO  cust_retpayhist (customer_id,customer_name,credit,pay_type,date) VALUES (:customer_id, :customer_name, :credit, :pay_type, :date)";
