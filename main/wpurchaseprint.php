...
{
    $result = $db->prepare("SELECT * FROM wpurchases_ret WHERE invoice_number = :invoice_number LIMIT :start, :perPage");
...
$result = $db->prepare("SELECT * FROM wpurchases_ret  WHERE invoice_number= :userid LIMIT 1");
...
$result = $db->prepare("SELECT COUNT(*) as total FROM wpurchases_ret WHERE invoice_number = :invoice");
...
try {
    ...
    for ($page = 1; $page <= $totalPages; $page++) {
        ...
        if (!empty($records)) {
            ...
            $pdf->Cell(0, 10, 'Purchase ', 0, 1, 'C'); // Change this line accordingly
