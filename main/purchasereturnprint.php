...
function getRecords($db, $invoiceNumber, $start, $perPage)
{
    ...
    $result = $db->prepare("SELECT * FROM purchases_ret WHERE return_invoice = :invoice_number LIMIT :start, :perPage");
...
$result = $db->prepare("SELECT * FROM purchases_ret WHERE return_invoice= :userid  LIMIT 1");
...
$result = $db->prepare("SELECT COUNT(*) as total FROM  purchases_ret WHERE return_invoice = :invoice  ");
