...
$suplier_id =  $_POST['suplier_id'];
 if ($reportType === 'daily') {
    ...
    echo "DATE<br/>";
    echo $date;
    $sql = "SELECT date, type, invoice_number, SUM(amount*quantity) AS total FROM wpurchases_ret WHERE   DATE_FORMAT(date, '%m/%d/%y') = :date AND suplier_id = :suplier_id GROUP BY type,invoice_number,date";
    $stmt = $db->prepare($sql);
    ...
    $sql = "SELECT date,type, invoice_number, SUM(amount*quantity) AS total FROM wpurchases_ret WHERE DATE_FORMAT(date, '%m/%d/%y') BETWEEN :start AND :end AND suplier_id = :suplier_id GROUP BY type,invoice_number,date";
    $stmt = $db->prepare($sql);
    ...
    $sql = "SELECT date,type, invoice_number, SUM(amount*quantity) AS total FROM wpurchases_ret WHERE RIGHT(DATE_FORMAT(date, '%m/%d/%y'), 2) = :year AND SUBSTRING(DATE_FORMAT(date, '%m/%d/%y'), 1, 2) = :month AND suplier_id = :suplier_id GROUP BY type,invoice_number,date";
    $stmt = $db->prepare($sql);
