...
<a href="#" onclick="window.print()" style="float:right;" class="btn btn-info"><i class="icon-print icon-large"></i> Print List</a>
    ...
    <form id="reportForm" method="post" action="wpurchasesreport.php" style="margin-left: 500px;">
...
$suplier_id =  $_POST['suplier_id'];
 if ($reportType === 'daily') {
    ...
    echo "DATE<br/>";
    echo $date;
    $sql = "SELECT date, type, invoice_number,suplier_id, SUM(amount*quantity) AS total FROM wpurchases_ret WHERE   DATE_FORMAT(date, '%m/%d/%y') = :date AND suplier_id = :suplier_id GROUP BY type,invoice_number,date,suplier_id";
    $stmt = $db->prepare($sql);
    ...
    $sql = "SELECT date,type, invoice_number, suplier_id, SUM(amount*quantity) AS total FROM wpurchases_ret WHERE DATE_FORMAT(date, '%m/%d/%y') BETWEEN :start AND :end AND suplier_id = :suplier_id GROUP BY type,invoice_number,date,suplier_id";
    $stmt = $db->prepare($sql);
    ...
    $sql = "SELECT date,type, invoice_number,suplier_id, SUM(amount*quantity) AS total FROM wpurchases_ret WHERE RIGHT(DATE_FORMAT(date, '%m/%d/%y'), 2) = :year AND SUBSTRING(DATE_FORMAT(date, '%m/%d/%y'), 1, 2) = :month AND suplier_id = :suplier_id GROUP BY type,invoice_number,date,suplier_id";
    $stmt = $db->prepare($sql);
...
<tr  class='record clickable-row' data-href='wpurchinvsumm.php?invoice_number=<?php echo $row['invoice_number']; ?>'>
