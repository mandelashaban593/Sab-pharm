...
<table class="table table-bordered" id="resultTable" data-responsive="table" style="text-align: left;">
	...
	<tbody>
			...
			<tr class="record">
			<td><?php
			$rrrrrrr=$row['gen_name'];
			$resultss = $db->prepare("SELECT * FROM products WHERE product_id= :asas");
			...
			<tr>
				...
				$sdsd=$_GET['product_id'];
				$resultas = $db->prepare("SELECT sum(o_price) FROM products WHERE product_id= :a");
				$resultas->bindParam(':a', $sdsd);
				$resultas->execute();
				for($i=0; $rowas = $resultas->fetch(); $i++){
				$fgfg=$rowas['sum(o_price)'];
