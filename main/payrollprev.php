...
<div style="margin: 0 auto; padding: 20px; width: 900px; font-weight: normal;">
	...
	<center><div style="font:bold 25px 'Aleo';">Payroll Receipt</div>
Ojinga Pharmacy	<br>
	Jinja,Uganda	<br>	<br>
	</center>
	...
	$result = $db->prepare("SELECT * FROM payroll WHERE invoice_number= :userid LIMIT 1");
	...
	<table border="1" cellpadding="4" cellspacing="0" style="font-family: arial; font-size: 12px;	text-align:left;" width="100%">
		...
		<tbody>
				...
				<?php
				  ...
				  if(isset($_GET['invoice'])) $id=$_GET['invoice'];
					$result = $db->prepare("SELECT * FROM payroll WHERE invoice_number= :userid");
					$result->bindParam(':userid', $id);
					$result->execute();
					for($i=0; $row = $result->fetch(); $i++){
				?>
				...
				<td><?php echo $row['hllowance']; ?></td>
				<td><?php echo $row['tllowance']; ?></td>
				<td><?php echo $row['advpayment']; ?></td>
				<td><?php echo $row['payee']; ?></td>
				<td><?php echo $row['nssf']; ?></td>
				<td><?php echo $row['nsalary']; ?></td>
				...
				<tr>
					...
					$resultas = $db->prepare("SELECT sum(nsalary) FROM payroll WHERE invoice_number= :a");
					$resultas->bindParam(':a', $sdsd);
					$resultas->execute();
					for($i=0; $rowas = $resultas->fetch(); $i++){
					$fgfg=$rowas['sum(nsalary)'];
