...
<a href="#" onclick="printPage('prodlist');" style="float:right;" class="btn btn-info"><i class="icon-print icon-large"></i> Medicine List</a>
			...
			<?php 
				...
				$result = $db->prepare("SELECT * FROM orders ORDER BY quantity_ordered DESC");
				$result->execute();
			...
			<?php 
				...
				$result = $db->prepare("SELECT * FROM orders where quantity_ordered < 10 ORDER BY product_id DESC");
				$result->execute();
				$rowcount123 = $result->rowcount();

			?>
				<div style="text-align:center;">
			Total Number of Medicines:  <font color="green" style="font:bold 22px 'Aleo';">[<?php echo $rowcount;?>]</font>
			</div>
...
<a rel="facebox" href="addmedorder.php"><Button type="submit" class="btn btn-info" style="float:right; width:230px; height:35px;" /><i class="icon-plus-sign icon-large"></i> Add Medicine Order</button></a><br><br><br/>
...
<table class="hoverTable" id="resultTable" data-responsive="table" style="text-align: left;">
	<thead>
		<tr>
			...
			<th width="14%"> quantity_ordered</th>
			...
			<th width="5%"> quantity_ordered sold </th>
	...
	<tbody>
			...
			function formatMoney($number, $fractional=false) {
				...
				$result = $db->prepare("SELECT *, price * quantity_ordered as total FROM orders ORDER BY med_name");
				$result->execute();
			...
			<td><?php if(!$row['qty_left'] ) {echo $row['quantity_ordered']; } else {echo $row['qty_left'];  } ?></td>
			...
			?>
						...
						<td><a rel="facebox" title="Click to edit the product" href="editmedorder.php?id=<?php echo $row['order_id']; ?>"><button class="btn btn-warning"><i class="icon-edit"></i> </button> </a>
