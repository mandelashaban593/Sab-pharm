...
$ovTotal =0;
 ...
 $sql = "SELECT date, type, invoice_number,acc_approve,man_approve, SUM(amount*quantity) AS total FROM purchases_rever GROUP BY type,invoice_number,acc_approve,man_approve,date ";
$stmt = $db->prepare($sql);
$stmt->execute();
...
<td><?php echo  $row['total'];  $ovTotal += $row['total'];  ?> </td>
<td><?php echo $row['acc_approve']; ?> </td>
<td><?php echo $row['man_approve']; ?> </td>

<td> <button onclick="approveAccStatus('<?php echo $row['invoice_number']; ?>', 'acc_approve')"  class="btn btn-success">1st Approval.</button>
<button onclick="approveManStatus('<?php echo  $row['invoice_number']; ?>', 'man_approve')"  class="btn btn-success">2nd Approval.</button></td>
...
<?php include('footer.php');?>
     ...
     <script>
          ...
          function approveAccStatus(invoice_number, status) {
            console.log("Invoice Number" + invoice_number);
            console.log("Status" + status);
            $.ajax({
                type: "POST",
                url: "update_acc_status.php",
                data: {invoice_number: invoice_number, status: status},
                success: function(response) {
                    // Update the status in the HTML after successful update in the database
                    console.log("Responss data" + response.content);
                    $("#" + invoice_number + "_" + status).text(response);
                    location.reload();
                }
            });
        }
         ...
         function approveManStatus(invoice_number, status) {
            console.log("Invoice Number" + invoice_number);
            console.log("Status" + status);
            $.ajax({
                type: "POST",
                url: "update_man_status.php",
                data: {invoice_number: invoice_number, status: status},
                success: function(response) {
                    // Update the status in the HTML after successful update in the database
                    console.log("Responss data", response);
