...
*/
			...
			$resulta =  mysqli_query($con, "SELECT sum(amount) as value_sum FROM sales WHERE curdate BETWEEN '$d1' AND '$d2' ORDER BY transaction_id DESC ") or die(mysqli_error($resulta));
		...
		/*	$results = $db->prepare("SELECT sum(amount) as value_sum FROM sales WHERE due_date BETWEEN :a AND :b");
			...
			echo $sum ;
				...
				?></td>
			<td><?php 
		$resulta =  mysqli_query($con, "SELECT sum(profit) as value_sum FROM sales WHERE curdate BETWEEN '$d1' AND '$d2'  ORDER BY transaction_id DESC ") or die(mysqli_error($resulta));
			$row_sum = mysqli_fetch_assoc($resulta);
