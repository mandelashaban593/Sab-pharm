...
*/
 ...
 try {
    ...
    $sql = "UPDATE wproducts SET quantity = CAST(quantity AS SIGNED) - :subtract_quantity WHERE product_id = :product_id";

    // Prepare the SQL statement
    $stmt = $db->prepare($sql);
    
    // Subtract the quantity as an integer
    $subtract_qty = (int)$qtytot;

    // Bind parameters
    $stmt->bindParam(':subtract_quantity', $subtract_qty);
...
$sql = "UPDATE wpurchases_ret  SET quantity = quantity - :quantity, total = total - :total WHERE transaction_id= :transaction_id";
...
header("location: wpurchasereturnprint.php?invoice=$return_invoice");
exit();
