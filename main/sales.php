...
<?php } ?>  
    ...
    <div class="contentheader">
            ...
            <li class="active">Retail Sale</li>
            </ul>
<div id="maintable">
...
<form id="productForm" method="post" action="savesales.php" style="width:250px;margin-top: 50px;">
        ...
        <table id="table-body" border="1"  style="width:150px">
            ...
            <tr>
                ...
                <td><input type="number" name="quantity[]" placeholder="Quantity"  class="quantity_input" required oninput="calRate(this); calAmount(this); calTot(this);checkQuantity(this, '1');checkTotQuantity(this, '1');  validateNumber(this, 'error_quantity1')" value="0" step="any"   >
                <span id="error_quantity1" class="error"></span>
                </td>
                <td><input type="number" name="price[]" placeholder="Rate" class="price_input" required  oninput="calAmount(this);  calTot(this);validateNumber(this, 'error_price1')"  value="0" id="rate" step="any"  >
                <span id="error_price1" class="error"></span>
                </td>
                <td><input type="number" name="amount[]" class="amount_input" placeholder="Amount"  oninput="calRate(this); calTot(this);validateNumber(this, 'error_amount1')" required value="0" id="amount" step="any"  >
...
function checkQuantity(inputElement, rowId) {
    ...
    var errorId = `error_quantity${rowId}`;
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            // Get the input fields for rate and amount
            var rateInput = inputElement.parentElement.nextElementSibling.querySelector('.price_input');
            var amountInput = inputElement.parentElement.nextElementSibling.nextElementSibling.querySelector('.amount_input');

            // Disable/enable the input fields based on the response
            if (response.trim() === '') {
                // If no error, hide the error element and enable the input fields
                document.getElementById(errorId).style.display = 'none';
                rateInput.disabled = false;
                amountInput.disabled = false;
            } else {
                // If there is an error, show the error element and disable the input fields
...
function checkTotQuantity(inputElement, rowId) {
    var medn = document.getElementsByName("med_name[]");
    var batno = document.getElementsByName("batch_no[]");
    var qty = document.getElementsByName("quantity[]");
    var productid = document.getElementsByName("productid[]");

    var row = inputElement.closest("tr");
    var curmedn = row.cells[0].getElementsByTagName("input")[0].value;
    var curbatno = row.cells[5].getElementsByTagName("input")[0].value;
    var curqty = row.cells[1].getElementsByTagName("input")[0].value;
    var curproductid = row.cells[4].getElementsByTagName("input")[0].value;
    ...
    for (var i = 0; i < medn.length; i++) {
        console.log("Medicine Name Tot Quantity", medn[i].value);
        console.log("Batch No Tot Quantity", batno[i].value);
        console.log("Quantity Tot Quantity", qty[i].value);
        console.log("Productid Tot Quantity", productid[i].value);
        if (medn[i].value == curmedn && batno[i].value == curbatno &&  productid[i].value == curproductid ) {
            savemedn.push(medn[i].value);
            savebatno.push(batno[i].value);
            saveqty.push(qty[i].value);
            saveproductid.push(productid[i].value);
        }
    }
    ...
    $.ajax({
        type: 'POST',
        url: 'checktotquantity.php',
        data: { savemedn: savemedn, savebatno: savebatno, saveqty: saveqty, saveproductid: saveproductid },
        success: function (response) {
            console.log("Response from PHP backend", response);
            document.getElementById(errorId).innerText = response;
            ...
            newRow.innerHTML = `
                ...
                <td>
                    <input type="number" name="quantity[]" placeholder="Quantity" class="quantity_input" required 
                        oninput="calRate(this); calAmount(this); calTot(this); checkQuantity(this, ${newRowNumber}); checkTotQuantity(this, ${newRowNumber});  validateNumber(this, 'error_quantity${newRowNumber}')"
                        value="0" step="any"  >
                ...
                <td>
                    <input type="number" name="price[]" placeholder="Rate" class="price_input" required 
                        oninput="calAmount(this); calTot(this); validateNumber(this, 'error_price${newRowNumber}')"
                ...
                <td>
                    <input type="number" name="amount[]" class="amount_input" placeholder="Amount" required 
                        oninput="calRate(this); calTot(this); validateNumber(this, 'error_amount${newRowNumber}')"
