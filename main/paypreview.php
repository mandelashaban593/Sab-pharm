...
{
    $result = $db->prepare("SELECT * FROM credit_payhist  WHERE transaction_id = :transaction_id LIMIT :start, :perPage");
...
$result = $db->prepare("SELECT COUNT(*) as total FROM credit_payhist  WHERE transaction_id  = :transaction_id");
...
try {
    ...
    $recordCount = 1;
        ...
        $records = $db->prepare("SELECT * FROM credit_payhist  WHERE transaction_id = :transaction_id ");
