...
foreach ($productid as $key => $pid) {
       ...
       $qty = $quantity[$key]; // Use a different variable for quantity
        ...
        $mednam = $med_name[$key];  // Use a different variable for product type
/*
   ...
   // echo "Number of rows retrieved: $numRows";
    ...
    $query = mysqli_query($con, "SELECT * FROM wproducts WHERE batch_no= '$batchno' and med_name='$mednam' ") or die(mysqli_error($con));
    $records=mysqli_fetch_array($query);
    // Get the number of rows
    $numBatch = mysqli_num_rows($query);

    if ($numBatch > 0) {
   // echo "Number of rows retrieved: $numBatch";
    ...
    if ($numBatch <= 0) {
        ...
        $sql = "INSERT INTO wproducts (product_code,category,med_name,price,supplier,quantity,o_price,profit,gen_name,product_name,cost,onhand_qty,qty,qty_sold,expiry_date,date_arrival,sell_type,reg_date,tot_buy,del_no,qty_left,status,batch_no) VALUES (:a,:b,:c,:e,:f,:g,:h,:i,:j,:k,:l,:m,:n,:o,:p,:k,:r,:s,:t,:u,:v,:w, :x)";
        $q = $db->prepare($sql);
      ...
      //  echo "The ID of the currently inserted record is: " . $lastInsertedId;
        ...
        $sql = "INSERT INTO wpurchases_ret (invoice_number,cashier,date,type,amount,profit,due_date,name, tme,productid,total,pay_type,quantity,exp_date,batch_no,suplier_id, med_name) VALUES ('$invoice','$cashier','$date','$ptype','$pri','$profit2',CURDATE(),'$cname',CURTIME(), '$lastInsertedId', '$amt', '$ptype', '$qty', '$expirydate', '$batchno', '$suplier_id', '$med_name3')";
...
if($ptype == "credit" ) {
    ...
    $checkQuery = "SELECT * FROM  wpurch_revenue  WHERE suplier_id = :suplier_id";
    ...
    if ($stmt->rowCount() > 0) {
        ...
        $updateQuery = "UPDATE wpurch_revenue SET credit = credit + :credit,suplier_name = :suplier_name,pay_type = :pay_type,date = :date WHERE suplier_id = :suplier_id";
        ...
        $insertQuery = "INSERT INTO wpurch_revenue (suplier_id,suplier_name, credit,pay_type, date) VALUES (:suplier_id,:suplier_name, :credit, :pay_type, :date)";
...
if($ptype == "cash" ) {
    ...
    if ($stmt->rowCount() > 0) {
        // The suplier_id exists; update the amount
        $updateQuery = "UPDATE wpurch_revenue SET cash = cash + :cash,suplier_name = :suplier_name,pay_type = :pay_type,date = :date WHERE suplier_id = :suplier_id";
        $stmt = $db->prepare($updateQuery);
        $stmt->bindParam(':suplier_id', $suplier_id, PDO::PARAM_INT);
         $stmt->bindParam(':suplier_name', $cname, PDO::PARAM_STR);
         $stmt->bindParam(':pay_type', $ptype, PDO::PARAM_STR);
        ...
        echo "Amount updated successfully.";
         ...
         $sql = "INSERT INTO wpurch_revenue (suplier_id,suplier_name,cash, pay_type,date)  VALUES (?, ?, ?, ?, ?)";
...
try {
    ...
    $sql = "INSERT INTO  wcredit_payhist (suplier_id,suplier_name,credit,pay_type,date) VALUES (:suplier_id, :suplier_name, :credit, :pay_type, :date)";
...
try {
    ...
    $sql = "INSERT INTO  wcredit_payhist (suplier_id,suplier_name,cash,pay_type,date) VALUES (:suplier_id, :suplier_name, :cash, :pay_type, :date)";
...
header("location: wpurchaseprint.php?invoice=$invoice");
exit();
