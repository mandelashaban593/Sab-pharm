...
try {
    ...
    if (isset($_FILES['excelFile']) && $_FILES['excelFile']['error'] == UPLOAD_ERR_OK) {
        ...
        try {
            ...
            $sql = "UPDATE wproducts SET status = 'Obsolete' and quantity=0";
            $db->exec($sql);
        ...
        for ($row = 2; $row <= $highestRow; $row++) { // Start from 2 to skip the header row
            ...
            $stmt = $db->prepare("SELECT * FROM wproducts WHERE med_name = :med_name  AND status = :status LIMIT 1");
            // Bind the parameter
          ...
          $stmt = $db->prepare("INSERT INTO wproducts (med_name, reg_date, batch_no,category, inventory, sell_type, expiry_date, price,status,stockval, quantity) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
