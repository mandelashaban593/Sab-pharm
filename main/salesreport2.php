...
$reportType = $_POST['report_type'];
 ...
 if ($reportType === 'daily') {
    ...
    echo "DATE<br/>";
    echo $date;
    $sql = "SELECT * FROM sales WHERE   DATE_FORMAT(date, '%m/%d/%y') = :date";
    $stmt = $db->prepare($sql);
    ...
    $sql = "SELECT * FROM sales WHERE DATE_FORMAT(date, '%m/%d/%y') BETWEEN :start AND :end";
    $stmt = $db->prepare($sql);
    ...
    $sql = "SELECT * FROM sales WHERE RIGHT(DATE_FORMAT(date, '%m/%d/%y'), 2) = :year AND SUBSTRING(DATE_FORMAT(date, '%m/%d/%y'), 1, 2) = :month";
    $stmt = $db->prepare($sql);
