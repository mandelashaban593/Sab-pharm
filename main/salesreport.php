...
<a href="#" onclick="window.print()" style="float:right;" class="btn btn-info"><i class="icon-print icon-large"></i> Print List</a>
    ...
    <form id="reportForm" method="post" action="salesreport.php" style="margin-left: 500px;">
        <h1>Sales Reports</h1>
...
$suplier_id =  $_POST['suplier_id'];
 if ($reportType === 'daily') {
    ...
    echo "DATE<br/>";
    echo $date;
    $sql = "SELECT date, pay_type, vouch_type,invoice_number,customer_id, SUM(amount*quantity) AS total FROM sales WHERE   DATE_FORMAT(date, '%m/%d/%y') = :date AND customer_id = :suplier_id GROUP BY pay_type, vouch_type,invoice_number,date,customer_id";
    $stmt = $db->prepare($sql);
    ...
    $sql = "SELECT date,pay_type, vouch_type, invoice_number,customer_id, SUM(amount*quantity) AS total FROM sales WHERE DATE_FORMAT(date, '%m/%d/%y') BETWEEN :start AND :end AND customer_id = :suplier_id GROUP BY pay_type, vouch_type,invoice_number,date,customer_id";
    $stmt = $db->prepare($sql);
    ...
    $sql = "SELECT date, pay_type, vouch_type,invoice_number,customer_id, SUM(amount*quantity) AS total FROM sales WHERE RIGHT(DATE_FORMAT(date, '%m/%d/%y'), 2) = :year AND SUBSTRING(DATE_FORMAT(date, '%m/%d/%y'), 1, 2) = :month AND customer_id = :suplier_id GROUP BY pay_type, vouch_type,invoice_number,date,customer_id";
    $stmt = $db->prepare($sql);
