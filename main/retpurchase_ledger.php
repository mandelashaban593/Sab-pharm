...
<i class="icon-bar-chart"></i> Retail Purchases Ledger
</div>
<ul class="breadcrumb">
<li><a href="index.php">Dashboard</a></li> /
<li class="active"> Retail Purchases Ledger</li>
</ul>
...
<a href="#" onclick="window.print()" style="float:right;" class="btn btn-info"><i class="icon-print icon-large"></i> Print List</a>
    ...
    <form id="reportForm" method="post" action="retpurchase_ledger.php" style="margin-left: 500px;">
...
$suplier_id =  $_POST['suplier_id'];
 if ($reportType === 'daily') {
    ...
    echo "DATE<br/>";
    echo $date;
    $sql = "SELECT date, type, invoice_number,suplier_id, vouch_type,return_invoice,SUM(amount*quantity) AS total FROM purchases_ret WHERE   DATE_FORMAT(date, '%m/%d/%y') = :date AND suplier_id = :suplier_id GROUP BY type,invoice_number,date,vouch_type,return_invoice,suplier_id";
    $stmt = $db->prepare($sql);
    ...
    $sql = "SELECT date,type, invoice_number,suplier_id,vouch_type,return_invoice, SUM(amount*quantity) AS total FROM purchases_ret WHERE DATE_FORMAT(date, '%m/%d/%y') BETWEEN :start AND :end AND suplier_id = :suplier_id GROUP BY type,invoice_number,date,vouch_type,return_invoice,suplier_id";
    $stmt = $db->prepare($sql);
    ...
    $sql = "SELECT date,type, invoice_number,suplier_id,vouch_type,return_invoice, SUM(amount*quantity) AS total FROM purchases_ret WHERE RIGHT(DATE_FORMAT(date, '%m/%d/%y'), 2) = :year AND SUBSTRING(DATE_FORMAT(date, '%m/%d/%y'), 1, 2) = :month AND suplier_id = :suplier_id GROUP BY type,invoice_number,date,vouch_type,return_invoice,suplier_id";
    $stmt = $db->prepare($sql);
...
<td><a rel="facebox" title="Click to print the invoice" href="purchaseprint.php?invoice=<?php echo $row['invoice_number']; ?>"><button class="btn btn-primary"><i class="icon-edit">Print Invoice</i> </button> </a>
</td>
<?php } else if($row['vouch_type']=='Credit Note') { ?>
<td><a rel="facebox" title="Click to print the invoice" href="purchasereturnprint.php?invoice=<?php echo $row['return_invoice']; ?>"><button class="btn btn-primary"><i class="icon-edit">Print Invoice</i> </button> </a>
</td>
<?php
