...
{
    $result = $db->prepare("SELECT * FROM wpurchases_ret WHERE return_invoice = :return_invoice LIMIT :start, :perPage");
...
$result = $db->prepare("SELECT * FROM wpurchases_ret  WHERE return_invoice= :userid LIMIT 1");
...
$result = $db->prepare("SELECT COUNT(*) as total FROM wpurchases_ret WHERE return_invoice = :invoice");
...
try {
    ...
    for ($page = 1; $page <= $totalPages; $page++) {
        ...
        if (!empty($records)) {
            ...
            $pdf->Cell(0, 10, 'Purchase Credit Note', 0, 1, 'C'); // Change this line accordingly
           ...
           // $recordCount = 0;
            ...
            foreach ($records as $record) {
                ...
                $pdf->SetFont('Arial', 'B', 10);
                $pdf->MultiCell(57, 6.8, $medicineInfo, 0, 'L');
                $pdf->SetXY($pdf->GetX() + 67, $pdf->GetY() - 10); // Adjust X and Y coordinates for the next line

                // Other cells
                $pdf->SetFont('Arial', 'B', 10);
                $pdf->Cell(30, 1, $record['quantity'], 0, 0, 'C');
                $pdf->SetFont('Arial', '', 14);
                $pdf->Cell(30, 1, formatMoney($record['amount']), 0, 0, 'C');
                $pdf->Cell(30, 1, $row2['sell_type'], 0, 0, 'C');
                $pdf->SetFont('Arial', 'B', 10);
                $pdf->Cell(30, 1, formatMoney($record['total']), 0, 1, 'C');
