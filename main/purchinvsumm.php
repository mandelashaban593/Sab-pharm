...
<?php
    ...
    echo $invoice_number;
    $sql = "SELECT * FROM purchases_ret WHERE   invoice_number = :invoice_number";
    $stmt = $db->prepare($sql);
...
<td><?php echo  formatMoney($row['quantity']*$row['amount']); ?> </td>
...
$sql = "SELECT SUM(amount*quantity) AS total FROM purchases_ret WHERE   invoice_number = :invoice_number";
$stmt = $db->prepare($sql);
$stmt->bindParam(':invoice_number', $invoice_number);
$stmt->execute();
