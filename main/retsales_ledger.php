...
<?php include('navfixed.php');?>
	...
	<div class="contentheader">
			...
			<li class="active">Retail Sales  Ledger</li>
			</ul>
<div id="maintable">
...
<th>Credit</th>
	...
	<tbody>
			...
			if(isset($_POST['tdate'])) $d2=date($_POST['tdate']);
               ...
               FROM  wsales WHERE date BETWEEN :a AND :b");
...
*/
        ...
        <td><?php echo formatMoney($row['Credit']); ?></td>
			...
			$resulta =  mysqli_query($con, "SELECT sum(amount) as value_sum FROM wsales WHERE date BETWEEN '$d1' AND '$d2' ORDER BY transaction_id DESC ") or die(mysqli_error($resulta));
			$row_sum = mysqli_fetch_assoc($resulta);

		/*	$results = $db->prepare("SELECT sum(amount) as value_sum FROM sales WHERE due_date BETWEEN :a AND :b");
			$results->bindParam(':a', $d1);
			$results->bindParam(':b', $d2);
			$results->execute();*/
			...
			echo $sum ;
				...
				?></td>
			<td><?php 
		$resulta =  mysqli_query($con, "SELECT sum(profit) as value_sum FROM wsales WHERE date BETWEEN '$d1' AND '$d2'  ORDER BY transaction_id DESC ") or die(mysqli_error($resulta));
			$row_sum = mysqli_fetch_assoc($resulta);
