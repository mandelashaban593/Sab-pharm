...
<li class="active">Retail Sale Ledger</li>
</ul>
...
<a href="#" onclick="window.print()" style="float:right;" class="btn btn-info"><i class="icon-print icon-large"></i> Print List</a>
    ...
    <form id="reportForm" method="post" action="retsales_ledger.php" style="margin-left: 500px;">
        <h1>Sales Reports</h1>
...
$suplier_id =  $_POST['suplier_id'];
 if ($reportType === 'daily') {
    ...
    echo "DATE<br/>";
    echo $date;
    $sql = "SELECT date, pay_type, vouch_type,return_invoice,invoice_number,customer_id, SUM(total) AS total FROM sales WHERE   DATE_FORMAT(date, '%m/%d/%y') = :date AND customer_id = :suplier_id GROUP BY pay_type, vouch_type,return_invoice, invoice_number,date,customer_id";
    $stmt = $db->prepare($sql);
    ...
    $sql = "SELECT date,pay_type, vouch_type,return_invoice, invoice_number,customer_id,SUM(total) AS total FROM sales WHERE DATE_FORMAT(date, '%m/%d/%y') BETWEEN :start AND :end AND customer_id = :suplier_id GROUP BY pay_type, vouch_type,return_invoice,invoice_number,date,customer_id";
    $stmt = $db->prepare($sql);
    ...
    $sql = "SELECT date, pay_type, vouch_type,return_invoice,invoice_number,customer_id,SUM(total) AS total FROM sales WHERE RIGHT(DATE_FORMAT(date, '%m/%d/%y'), 2) = :year AND SUBSTRING(DATE_FORMAT(date, '%m/%d/%y'), 1, 2) = :month AND customer_id = :suplier_id GROUP BY pay_type, vouch_type,return_invoice,invoice_number,date,customer_id";
    $stmt = $db->prepare($sql);
...
<td><?php echo  formatMoney($row['total']); if($row['vouch_type']=='Sales') $salesTotal +=  $row['total'];  if($row['vouch_type']=='Credit Note')  $credNoteTotal += $row['total'];  ?> </td>

<?php if($row['vouch_type']=='Sales') { ?>
<td><a rel="facebox" title="Click to print the invoice" href="retailsaleprint.php?invoice=<?php echo $row['invoice_number']; ?>"><button class="btn btn-primary"><i class="icon-edit">Print Invoice</i> </button> </a>
</td>
<?php } else if($row['vouch_type']=='Credit Note') { ?>
<td><a rel="facebox" title="Click to print the invoice" href="retailsalereturnprint.php?invoice=<?php echo $row['return_invoice']; ?>"><button class="btn btn-primary"><i class="icon-edit">Print Invoice</i> </button> </a>
</td>
<?php
...
} //End if statement for post request ?>
