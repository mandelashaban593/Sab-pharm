...
<li class="active">Whole Sale Ledger</li>
</ul>
...
<a href="#" onclick="window.print()" style="float:right;" class="btn btn-info"><i class="icon-print icon-large"></i> Print List</a>
    ...
    <form id="reportForm" method="post" action="wsales_ledger.php" style="margin-left: 500px;">
        <h1>Sales Reports</h1>
...
$suplier_id =  $_POST['suplier_id'];
 if ($reportType === 'daily') {
    ...
    echo "DATE<br/>";
    echo $date;
    $sql = "SELECT date, pay_type, vouch_type,return_invoice,invoice_number,customer_id, SUM(total) AS total FROM wsales WHERE   DATE_FORMAT(date, '%m/%d/%y') = :date AND customer_id = :suplier_id GROUP BY pay_type, vouch_type,return_invoice, invoice_number,date,customer_id";
    $stmt = $db->prepare($sql);
    ...
    $sql = "SELECT date,pay_type, vouch_type,return_invoice, invoice_number,customer_id,SUM(total) AS total FROM wsales WHERE DATE_FORMAT(date, '%m/%d/%y') BETWEEN :start AND :end AND customer_id = :suplier_id GROUP BY pay_type, vouch_type,return_invoice,invoice_number,date,customer_id";
    $stmt = $db->prepare($sql);
    ...
    $sql = "SELECT date, pay_type, vouch_type,return_invoice,invoice_number,customer_id,SUM(total) AS total FROM wsales WHERE RIGHT(DATE_FORMAT(date, '%m/%d/%y'), 2) = :year AND SUBSTRING(DATE_FORMAT(date, '%m/%d/%y'), 1, 2) = :month AND customer_id = :suplier_id GROUP BY pay_type, vouch_type,return_invoice,invoice_number,date,customer_id";
    $stmt = $db->prepare($sql);
...
<td><a rel="facebox" title="Click to print the invoice" href="wsaleprint.php?invoice=<?php echo $row['invoice_number']; ?>"><button class="btn btn-primary"><i class="icon-edit">Print Invoice</i> </button> </a>
</td>
