<?php
	...
	$result = $db->prepare("SELECT * FROM  advanced_payment WHERE payment_id= :userid");
	$result->bindParam(':userid', $id);
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
<link href="../style.css" media="screen" rel="stylesheet" type="text/css" />
<form action="saveeditadvancedpay.php" method="post">
<center><h4><i class="icon-edit icon-large"></i> Edit Advance Payment</h4></center>
<hr>
<div id="ac">
...
<span>Date. : </span><input type="date" style="width:265px; height:30px;" name="date" id="date" value="<?php echo $row['payment_date']; ?>"  Required/><br>
