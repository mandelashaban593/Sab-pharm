...
<form id="productForm" method="post" action="savewholesale.php" style="width:250px;margin-top: 50px;">
        ...
        <table id="table-body" border="1"  style="width:150px">
            ...
            <tr>
                ...
                <td><input type="number" name="quantity[]" placeholder="Quantity"  class="quantity_input" required oninput="calRate(this); calAmount(this); calTot(this);checkQuantity(this, 'error_quantity1'); validateNumber(this, 'error_quantity1')" value="0" >
                <span id="error_quantity1" class="error"></span>
                </td>
                <td><input type="number" name="price[]" placeholder="Rate" class="price_input" required  oninput="calAmount(this); calQuantity(this); calTot(this);validateNumber(this, 'error_price1')"  value="0" id="rate">
                <span id="error_price1" class="error"></span>
                </td>
                <td><input type="number" name="amount[]" class="amount_input" placeholder="Amount"  oninput="calRate(this); calQuantity(this); calTot(this);validateNumber(this, 'error_amount1')" required value="0" id="amount">
...
function validateNumber(input, errorId) {
 ...
 function checkQuantity(inputElement,  errorId) {
        ...
        //var quantityError = inputElement.parentElement.querySelector(`#error_quantity${newRowNumber}`);
        ...
        $.ajax({
            ...
            //    document.getElementById(errorId).innerText = 'Quantity available is less than zero';
            if (response === 'qtyavailess') {
                document.getElementById(errorId).innerText = 'Quantity available is less than zero';
                alert("Quantity available is less than zero");
            } else if (response === 'lessqty') {
                document.getElementById(errorId).innerText = 'Available quantity is less than the quantity entered';
                alert("Available quantity is less than the quantity entered");
                console.log('Less than ', response);
            } else if (response === 'inpqtygreater') {
                document.getElementById(errorId).innerText = 'Quantity entered is greater than the available quantity.';
            ...
            error: function () {
               document.getElementById(errorId).innerText  = 'Error checking quantity.';
            }
        });
    }
