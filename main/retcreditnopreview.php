...
Ojinga Pharmacy	<br>
	...
	$result = $db->prepare("SELECT * FROM retcreditnote  WHERE invoice_number= :userid LIMIT 1");
	...
	<table border="1" cellpadding="4" cellspacing="0" style="font-family: arial; font-size: 12px;	text-align:left;" width="100%">
		...
		<tbody>
				...
				<?php
				  ...
				  if(isset($_GET['invoice'])) $id=$_GET['invoice'];
					$result = $db->prepare("SELECT * FROM retcreditnote WHERE invoice_number= :userid");
					$result->bindParam(':userid', $id);
					$result->execute();
					for($i=0; $row = $result->fetch(); $i++){
				?>
				...
				<tr>
					...
					$resultas = $db->prepare("SELECT sum(amount) FROM retcreditnote  WHERE invoice_number= :a");
