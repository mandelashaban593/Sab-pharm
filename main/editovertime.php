<?php
	...
	$result = $db->prepare("SELECT * FROM overtime WHERE overtime_id= :userid");
	$result->bindParam(':userid', $id);
	$result->execute();
	for($i=0; $row = $result->fetch(); $i++){
?>
<link href="../style.css" media="screen" rel="stylesheet" type="text/css" />
<form action="saveeditovertime.php" method="post">
<center><h4><i class="icon-edit icon-large"></i> Edit Overtime</h4></center>
<hr>
<div id="ac">
...
<span>Hours worked: </span><input type="number" style="width:265px; height:30px;" name="hours_worked" id="hours_worked" value="<?php echo $row['hours_worked']; ?>"  Required/><br>
<span>Date. : </span><input type="date" style="width:265px; height:30px;" name="date" id="date" value="<?php echo $row['overtime_date']; ?>"  Required/><br>
